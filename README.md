...
- **MemoryLeaks** [KSP 1.12.0 - 1.12.3]<br/>Fix a bunch of managed memory leaks, mainly by proactively removing `GameEvents` delegates originating from destroyed `MonoBehaviour` instances. Will log detected leaks and real memory usage on scene switches, see `Settings.cfg` to enable more detailed logging.
