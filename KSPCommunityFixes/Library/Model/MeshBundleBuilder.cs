...
/// <c>AssetBundle</c> that references them.
/// </summary>
/// <remarks>
/// <see cref="WriteMeshBody"/> reproduces Unity 2019.4.18f1's serialized <c>Mesh</c> (class 43)
/// field order, sizes and alignment exactly as the embedded type tree encodes them.
/// </remarks>
internal static class MeshBundleBuilder
{
    ...
    /// <summary>
    /// Canonicalize an asset name into the format unity expects internally.
    /// </summary>
    public static string Canonicalize(string name) =>
        (name ?? string.Empty).ToLowerInvariant().Replace('\\', '/');

    /// <summary>
    /// Build one UnityFS bundle wrapping every mesh in <paramref name="blobs"/> plus
    /// the one <c>AssetBundle</c> object. Returns <c>null</c> when <paramref name="blobs"/> is empty.
    /// Throws if two blobs have the same canonical name.
    /// </summary>
    ...
    )
    {
        ...
        // multimap, so a colliding key would silently resolve to an arbitrary mesh).
    ...
    static void WriteMeshBody(BundleBufferWriter w, MeshBlob mesh)
    {
        ...
        // m_BoneNameHashes : vector<uint>, Array aligns. One CRC32 per bone.
        var boneNameHashes = w.BeginArray();
        uint[] hashes = mesh.BoneNameHashes ?? [];
        ...
        MeshBoneAABB[] aabbs = mesh.BonesAABB ?? [];
