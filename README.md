...
- **ExtendedDeployableParts** [KSP 1.12.0 - 1.12.5]<br/>Fix deployable parts (antennas, solar panels, radiators...) always starting in the extended state when the model isn't exported in the retracted state. This bug affect parts from various mods (ex : Ven's stock revamp solar panels). Also fixes a `NullReferenceException` when a static (non-animated) solar panel is placed on a part that has other, unrelated animations.
...
- **[SymmetryReferenceOnDelete](https://github.com/KSPModdingLibs/KSPCommunityFixes/issues/382)** [KSP 1.8.0 - 1.12.5]<br/>Fix deleting a part in the editor leaving stale (destroyed) references in the `symmetryCounterparts` list of surviving parts (after removing symmetry from a parent part that still has children), which broke ship saving and the editor.
...
- [**PatchedConicEncounterFlickering**](https://github.com/KSPModdingLibs/KSPCommunityFixes/issues/320) [KSP 1.8.0 - 1.12.5]<br/>Revert back to the pre-1.8 SOI encounter solver. This is slower but less likely to cause flickering.
- **ModernTargeting** [KSP 1.12.5]<br/>Fix incorrect angle wrapping and runaway recursion in the modern closest-approach solver that can cause crashes when selecting a target or editing a maneuver. Keeps modern targeting calculations; does not change the legacy targeting setting. Remove the standalone experimental TargetingGuard plugin before using this patch.
...
- [**EVAConstructionUninitializedInventory**](https://github.com/KSPModdingLibs/KSPCommunityFixes/issues/378) [KSP 1.12.3 - 1.12.5]<br/>Fixes a bug where stock will return parts that are not initialized yet when searching for parts with inventories, causing bugs in downstream mods.
...
- [**BlockMapViewPartClick**](https://github.com/KSPModdingLibs/KSPCommunityFixes/pull/362) [KSP 1.12.0 - 1.12.5]<br/>Fix mouse double clicks passing through to parts on the vessel while the map view is open, which would unselect the current target vessel or celestial body.
...
- **BaseFieldUIControlCollision** [KSP 1.12.0 - 1.12.5]<br/>Fix Part Action Window widgets being taken from the wrong field. `BaseField` caches the `UI_Control` attributes declared on a field in a dictionary keyed by `FieldInfo.GetHashCode()`, and a hash code isn't an identity. Depending on the control types involved, this results in a wrong PAW widget, or PAW breaking with exception spam.
...
- [**AsteroidAndCometDrillCache**](https://github.com/KSPModdingLibs/KSPCommunityFixes/pull/67) [KSP 1.12.5]<br/>Reduce constant overhead of ModuleAsteroidDrill and ModuleCometDrill by caching the vessel-wide asteroid/comet part lookups they perform on every update. Improves performance with large part count vessels having at least one drill part.
...
- [**GameDatabasePerf**](https://github.com/KSPModdingLibs/KSPCommunityFixes/pull/269) [KSP 1.12.3 - 1.12.5]<br/>Faster dictionary backed version of the stock `GameDatabase.GetModel*` / `GameDatabase.GetTexture*` / `GameDatabase.*AudioClip` methods. This patch is always enabled and has no entry in `Settings.cfg`.
...
- [**FlightIntegratorPerf**](https://github.com/KSPModdingLibs/KSPCommunityFixes/pull/267) [KSP 1.12.3 - 1.12.5]<br/>General micro-optimization of `FlightIntegrator` and `VesselPrecalculate` (`CalculatePhysicsStats`, `Integrate`, `UpdateAerodynamics`, `UpdateMassStats`, occlusion and radiation passes). Significant impact in large part count situations, especially in atmospheric flight.
- [**ActiveRadiatorPerf**](https://github.com/KSPModdingLibs/KSPCommunityFixes/pull/375) [KSP 1.12.0 - 1.12.5]<br/>Replace the per-module `ModuleActiveRadiator.FixedUpdate` work with a per-vessel batched heat integrator that runs once just before `FlightIntegrator`, and cache the relevant part/module lookups. Significantly reduces the overhead of active radiators on large vessels.
- [**CargoBayPerf**](https://github.com/KSPModdingLibs/KSPCommunityFixes/issues/356) [KSP 1.12.0 - 1.12.5]<br/>Defer `ModuleCargoBay.onVesselModified` recomputation (`EnableShieldedVolume`) until after the physics update completes, deduplicating bursts of `onVesselWasModified` events. Significantly reduces stutter when a ship with cargo bays crashes or breaks apart.
...
- [**EventProfilerMarkers**](https://github.com/KSPModdingLibs/KSPCommunityFixes/pull/355) [KSP 1.12.0 - 1.12.5]<br/>Disabled by default, you can enable it with a MM patch. Adds Unity Profiler markers to `EventData.Fire()` methods, making game event dispatch and individual subscriber callbacks visible in the Unity Profiler. Useful for identifying expensive event handlers.
